...
mod <- summary(coxph(formula(paste0("Surv(sos_outtime_hosphf, sos_out_deathhosphf == 'Yes') ~ ablation + ", paste(modvarsns, collapse = " + "))),
  data = pop
))
...
modm <- coxme(Surv(sos_outtime_hosphf, sos_out_deathhosphf == "Yes") ~ ablation + (1 | par),
  data = matchpop
)
se <- sqrt(vcov(modm)[1])
